...
@section('title', 'Roles Management - HappilyWeds')
...
<div class="container-fluid py-5 page-spacing font-sans">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3 animate-card">
        <div>
            <h2 class="font-serif fw-bold m-0 text-gradient" style="font-size: 2.5rem;">Roles Management</h2>
            <p class="text-muted mt-2 mb-0" style="font-size: 1rem; font-weight: 500;">Define user roles and assign precise system permissions.</p>
        </div>

        <a href="{{ route('admin.roles.create') }}" class="btn-glow shadow-sm d-inline-flex align-items-center text-decoration-none">
        ...
        <div class="table-responsive" style="overflow-x: auto; padding-bottom: 20px;">
            <table class="table premium-table w-100">
                ...
                <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <h6 class="mb-0 fw-bold text-dark fs-5 text-capitalize">
                                            {{ $role->name }}
                                        </h6>
                                    </div>
                                </div>
                            ...
                            <td class="py-3">
                                <div class="d-flex flex-wrap gap-2">
                                    @forelse($role->permissions as $perm)
                                        <span class="badge rounded-pill fw-bold shadow-sm" style="background: rgba(231, 84, 128, 0.08); color: #e75480; border: 1px solid rgba(231, 84, 128, 0.2); padding: 8px 14px; font-size: 0.75rem;">
                                            <i class="bi bi-check2-circle me-1"></i> {{ $perm->name }}
                                        </span>
                                    @empty
                                        <span class="badge rounded-pill fw-bold" style="background: #f8fafc; color: #64748b; border: 1px solid #e2e8f0; padding: 8px 16px; font-size: 0.8rem;">
                            ...
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end align-items-center">
                                    <a href="{{ route('admin.roles.edit', $role->id) }}" class="action-icon-btn edit" title="Edit Role">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>

                                    <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this role? Users assigned to this role may lose access.')">
                                        @csrf
                                        @method('DELETE')
                                        <button class="action-icon-btn delete" title="Delete Role">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-5 bg-transparent shadow-none border-0">
                                <div class="d-flex flex-column align-items-center justify-content-center py-5">
                                    ...
                                    <h4 class="font-serif fw-bold text-dark">No Roles Found</h4>
                                    <p class="text-muted fw-medium fs-6">Click 'Add New Role' to start defining access levels.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
