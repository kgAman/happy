...
@section('title', 'Country Code Master - HappilyWeds')
...
<div class="container-fluid py-5 page-spacing font-sans">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3 animate-card">
        <div>
            <h2 class="font-serif fw-bold m-0 text-gradient" style="font-size: 2.5rem;">Country Code Master</h2>
            <p class="text-muted mt-2 mb-0" style="font-size: 1rem; font-weight: 500;">Manage global dialing codes and country configurations.</p>
        </div>
    </div>
    ...
    <div class="premium-card p-4 mb-5 animate-card delay-1">
        <form method="POST" action="{{ route('admin.country-codes.store') }}">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-md-3">
                    <label class="premium-label">Country Name</label>
                    <input type="text" name="country_name" class="form-control premium-input" placeholder="e.g. India" required>
                </div>
                <div class="col-md-3">
                    <label class="premium-label">ISO Code</label>
                    <input type="text" name="country_code" class="form-control premium-input" placeholder="e.g. IN" required>
                </div>
                <div class="col-md-3">
                    <label class="premium-label">Dial Code</label>
                    <div class="position-relative">
                        <i class="bi bi-telephone position-absolute text-muted fs-5" style="left: 15px; top: 12px;"></i>
                        <input type="text" name="dial_code" class="form-control premium-input w-100" style="padding-left: 45px;" placeholder="e.g. +91" required>
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <tbody>
                    @forelse($data as $row)
                        <tr>
                            ...
                            <td>
                                <span class="badge rounded-pill fw-bold shadow-sm" style="background: #f8fafc; color: #475569; border: 1px solid #e2e8f0; padding: 6px 12px; font-size: 0.8rem; letter-spacing: 1px;">
                                    {{ $row->country_code }}
                                </span>
                            </td>
                            ...
                            <td class="text-end pe-4">
                                <form action="{{ route('admin.country-codes.destroy', $row->id) }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Are you sure you want to delete this country code?')">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" class="action-icon-btn delete" title="Delete Country Code">
                            ...
                            <td colspan="6" class="text-center py-5 bg-transparent shadow-none border-0">
                                <div class="d-flex flex-column align-items-center justify-content-center py-4">
                                    <div class="rounded-circle bg-white shadow-sm d-flex align-items-center justify-content-center mb-4" style="width: 80px; height: 80px;">
                                        <i class="bi bi-globe-x text-muted" style="font-size: 2.5rem;"></i>
                                    </div>
                                    <h4 class="font-serif fw-bold text-dark">No Country Codes Found</h4>
                                    <p class="text-muted fw-medium fs-6">Use the form above to add your first dialing code.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
