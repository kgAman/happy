...
@section('title', 'Occupation Master - HappilyWeds')
...
<div class="container-fluid py-5 page-spacing font-sans" style="max-width: 1000px;">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3 animate-card">
        <div>
            <h2 class="font-serif fw-bold m-0 text-gradient" style="font-size: 2.5rem;">Occupation Master</h2>
            <p class="text-muted mt-2 mb-0" style="font-size: 1rem; font-weight: 500;">Manage the professional occupations available for candidate profiles.</p>
        </div>
    </div>
    ...
    <div class="premium-card p-4 mb-5 animate-card delay-1">
        <form method="POST" action="{{ route('admin.occupations.store') }}">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-md-9">
                    <label class="premium-label">Occupation Name</label>
                    <div class="position-relative">
                        <i class="bi bi-briefcase position-absolute text-muted fs-5" style="left: 15px; top: 12px;"></i>
                        <input type="text" name="occupation" class="form-control premium-input w-100" style="padding-left: 45px;" placeholder="e.g. Software Engineer, Doctor, Teacher" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <button type="submit" class="btn-glow w-100 d-flex justify-content-center align-items-center" style="height: 48px;">
                        <i class="bi bi-plus-lg me-2"></i> Add Occupation
                    </button>
                </div>
            </div>
        </form>
                    ...
                    @forelse($data as $row)
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                <form action="{{ route('admin.occupations.destroy', $row->id) }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Are you sure you want to delete this occupation?')">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" class="action-icon-btn delete" title="Delete Occupation">
                            ...
                            <td colspan="4" class="text-center py-5 bg-transparent shadow-none border-0">
                                <div class="d-flex flex-column align-items-center justify-content-center py-4">
                                    <div class="rounded-circle bg-white shadow-sm d-flex align-items-center justify-content-center mb-4" style="width: 80px; height: 80px;">
                                        <i class="bi bi-briefcase-x text-muted" style="font-size: 2.5rem;"></i>
                                    </div>
                                    <h4 class="font-serif fw-bold text-dark">No Occupations Found</h4>
                                    <p class="text-muted fw-medium fs-6">Use the form above to add your first occupation.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
